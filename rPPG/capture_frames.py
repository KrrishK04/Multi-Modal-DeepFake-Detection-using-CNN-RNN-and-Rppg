...
class CaptureFrames():
    ...
    def capture_frames(self, source):
        ...
        # Check if source is a file path (string) or webcam (integer)
        if isinstance(source, str) and source.endswith(('.mp4', '.avi', '.mov')):  
            print(f"Attempting to open video file: {source}")
            camera = cv2.VideoCapture(source)
        else:
            print(f"Attempting to open camera with source: {source}")
        ...
        # print(f"Attempting to open camera with source: {source}")  # Debugging
        # camera = cv2.VideoCapture(int(source))
        # if not camera.isOpened():
